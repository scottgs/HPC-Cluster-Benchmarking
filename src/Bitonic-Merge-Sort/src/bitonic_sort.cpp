...
void sendMaxReceiveMin(vector<SortType> & vec, const int otherRank) {
	...
	CompareType tempCompare = CompareType(vec[pivot]),
				otherValue = -1; // just init to this--will get updated. This is the other rank's value used for comparison

	cout << "Pivot in sendMaxReceiveMin is: " << pivot << endl;
	cout << "Other rank in sendMaxReceiveMin is: " << otherRank << endl;
	...
	MPI_Sendrecv(
		...
		MIN_TAG,					// Send tag
		&otherValue,			// Init address of receive buffer
...
void sendMinReceiveMax(vector<SortType> & vec, const int otherRank) {
	...
	cout << "Pivot in sendMinReceiveMax is: " << pivot << endl;
	cout << "Other rank in sendMinReceiveMax is: " << otherRank << endl;
...
inline void bitonicSort(vector<SortType> & vec, const int numProcs, const int rank) {
	...
	const uint num_steps = log2(numProcs);

	for (int bitIndex = 1; bitIndex <= num_steps; ++bitIndex) {
		// Window ID is the most signiciant (d-i) bits
		const int diBit = (rank >> bitIndex) & 0x1;
		for (int j = bitIndex - 1; j >= 0; --j) {

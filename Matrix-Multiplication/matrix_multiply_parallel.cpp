...
double run_master(mpi::communicator world, const uint size, const uint grid_dimension, Matrix & result) {
	...
	// Otherwise, parallelize.
		...
		uint slave_id = 1;
		const uint sub_matrix_sizes = size / grid_dimension;

		for (uint i = 0; i < size; i += sub_matrix_sizes) {
			for (uint j = 0; j < size; j += sub_matrix_sizes) {
				Matrix_Section section = get_section(A, i, j, sub_matrix_sizes);
